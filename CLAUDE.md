...
**Cache invalidation is a correctness requirement, but caches are derived views, not the final source of truth for detail routes.** `article_summaries_cache` drives public article lists, tag pages, archive grouping, admin article tables, pagination counts, and the feed's top-level `updated_at`. `page_titles_cache` is injected into every template render via `AppState::render_template` and drives the navbar. Concrete `/article/{id_or_slug}` and `/{page}` requests must still let the detail DB query decide whether the entity exists; do not return 404 solely from a cache miss unless the cache-loading path has first been redesigned to be a fallible, authoritative index.

**Mutation handlers refresh only the caches their entity can affect.** `Article` mutations refresh `article_summaries_cache` and then rebuild `feed_cache`; `Page` mutations refresh only `page_titles_cache`. This is encoded on the `Editable` implementations through `REFRESH_ARTICLE_CACHES` / `REFRESH_PAGE_TITLES_CACHE` and applied by `handlers::refresh_caches_after_mutation`. New editable entities should set only the flags they actually need.
