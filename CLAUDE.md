...
`.github/workflows/neon-test.yml` owns the PostgreSQL-backed CI gate. On pushes to `main`, it first validates the required Neon secret names, then runs `cargo fmt --check` and `cargo clippy -- -D warnings` before creating a temporary Neon branch from the production Neon branch. It sets the branch to expire after one day, passes the branch's pooled connection string as `TEST_DATABASE_URL`, runs `cargo test -- --test-threads=1`, and deletes the temporary branch. It needs these GitHub Actions secrets: `NEON_API_KEY`, `NEON_PROJECT_ID`, `NEON_PRODUCTION_BRANCH_ID`, `NEON_DATABASE`, and `NEON_ROLE`. Keep the test command serial because the DB tests truncate shared tables.

`.github/workflows/fly-deploy.yml` owns deployment only. It is triggered by `workflow_run` after `Test with Neon Branch` completes on `main`, and the deploy job additionally checks `github.event.workflow_run.conclusion == 'success'` before running. It checks out `github.event.workflow_run.head_sha`, so Fly deploys the exact commit that passed the Neon-backed test workflow rather than whichever commit happens to be current when the deploy workflow starts.
...
- **Public**: `/` (home, paginated via `/page/{num}`), `/article/{id_or_slug}`, `/articles`, `/tag/{tag}`, `/tags`, `/feed`, `/ping`, `/healthz/db`, `/login`, `/logout`, and a catch-all `/{page}` that resolves a custom `Page` by title (case-insensitive, via `LOWER(title) = LOWER(?)`).
...
- `db: models::DbPool` — an enum wrapping either `sqlx::MySqlPool` or `sqlx::PgPool`. Backend is picked at `DbPool::connect` time from the URL scheme. `DbPool::connect` configures the pool explicitly for Neon serverless billing: `min_connections(0)`, a 15s `acquire_timeout` (to absorb Neon compute wake-up latency), a 60s `idle_timeout` (so idle connections release and the compute can scale to zero), and a small `max_connections` (3 against the Neon pooled `-pooler.neon.tech` endpoint, which already multiplexes via PgBouncer; 5 otherwise). `is_neon_pooler_url` detects the pooler host. Prepared-statement behavior is left at sqlx defaults — do not enable `simple_protocol`, it breaks the `.bind()`-based parameterized queries used throughout the models.
- `feed_cache` + `article_summaries_cache` + `page_titles_cache` — all `Arc<RwLock<...>>`.
- `article_content_cache` + `page_content_cache` — `Arc<RwLock<HashMap<String, ...>>>` keyed by the raw `id_or_slug` segment (articles) and the lowercased title (pages). They cache full bodies for the `/article/{id_or_slug}` and `/{page}` detail routes so repeat public views do not hit the DB. They start empty on boot (no eager load) and are backfilled on first miss, then cleared wholesale on the corresponding mutation.

**Cache invalidation is a correctness requirement, but caches are derived views, not the final source of truth for detail routes.** `article_summaries_cache` drives public article lists, tag pages, archive grouping, admin article tables, pagination counts, and the feed's top-level `updated_at`. `page_titles_cache` is injected into every template render via `AppState::render_template` and drives the navbar. `article_content_cache` / `page_content_cache` serve `/article/{id_or_slug}` and `/{page}` bodies. Concrete `/article/{id_or_slug}` and `/{page}` requests must still let the detail DB query decide whether the entity exists on a cache miss; do not return 404 solely from a cache miss unless the cache-loading path has first been redesigned to be a fallible, authoritative index.

**Mutation handlers refresh only the caches their entity can affect.** `Article` mutations refresh `article_summaries_cache`, rebuild `feed_cache`, and clear `article_content_cache`; `Page` mutations refresh `page_titles_cache` and clear `page_content_cache`. This is encoded on the `Editable` implementations through `REFRESH_ARTICLE_CACHES` / `REFRESH_PAGE_TITLES_CACHE` and applied by `handlers::refresh_caches_after_mutation`. The content caches are cleared wholesale rather than invalidated per-key (the entity set is small and admin writes are rare), so the next detail view re-queries and re-backfills. New editable entities should set only the flags they actually need.

**Feed refresh is deliberately unconditional after article mutations.** Admin writes are rare, and rebuilding the feed avoids subtle timestamp precision bugs where an insert/update in the same DB timestamp tick could otherwise skip a needed refresh. The feed's top-level `updated_at` value is derived from `article_summaries_cache`; the feed entries still query recent full articles because they need article content.
...
- `models::init_schema(&DbPool)` dispatches into the right one and issues the DDL through a transaction handle. Both files are idempotent via `IF NOT EXISTS`, so it is safe to call on every boot. Do not treat that as a cross-backend atomic migration guarantee: MySQL can implicitly commit DDL, so partial progress is still possible if a later statement fails. `models::init_schema_if_needed(&DbPool)` is what `AppState::new` actually calls on boot: it runs a single `COUNT(*)` round-trip checking that **all** tables in `SCHEMA_TABLES` (`articles`, `tags`, `pages`, `users`) exist, and only runs the full `init_schema` DDL when the count comes up short. This cuts cold-start DB round-trips from eight to one, a Neon CU-hrs saving, while preserving `init_schema`'s self-healing: a partially-dropped schema (e.g. only `tags` missing) still triggers the full idempotent DDL. Indexes are intentionally not probed — a missing index degrades performance but not correctness, and `CREATE INDEX IF NOT EXISTS` only runs on the full-DDL path. If a table is added to the schema files, add it to `SCHEMA_TABLES` **and** the `IN (...)` lists in `all_schema_tables_exist` so the probe stays in sync.
...
CI tests every push to `main` with `.github/workflows/neon-test.yml`, then deploys to Fly.io through `.github/workflows/fly-deploy.yml` only after that workflow succeeds. The deploy workflow stages `DATABASE_URL` and `UMAMI_ID` as Fly secrets, then runs `fly deploy --remote-only`. The matching GitHub Actions secret must be configured under the `DATABASE_URL` name before deploys; the workflow has no fallback. The Fly app is configured in `fly.toml` (region `nrt`, internal port `5299`, `/ping` healthcheck, `min_machines_running = 0` with auto-suspend). Because the session store is in-process, suspensions and redeploys both log admin sessions out — acceptable for a single-admin blog, but a blocker for any future multi-tenant mode or multi-machine rollout.

**Healthchecks and Neon CU-hrs.** `fly.toml`'s `/ping` healthcheck is probed every 60s while the Fly machine is up. `handler_ping` deliberately does NOT query the database — it only checks `state.db.is_closed()` (no network round-trip) and returns `pong`. This is load-bearing for cost: a periodic DB query from the healthcheck would keep resetting the Neon compute's idle timer and prevent it from scaling to zero, which is the dominant CU-hrs cost driver. DB liveness is instead exposed at `/healthz/db` (a public, non-Fly-probe route that runs `User::try_check_initialization`), intended for ad-hoc or external monitoring. Do not re-add a DB query to `handler_ping` without an explicit cost decision.
